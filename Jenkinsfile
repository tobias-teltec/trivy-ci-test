...
agent any
  ...
  stages {
      ...
      stage("Trivy Scan") {
          steps {
              script {
                        sh 'docker run -v /var/run/docker.sock:/var/run/docker.sock -v "$WORKSPACE"/trivycache:/root/.trivycache/ aquasec/trivy --exit-code 0 --format template --template "@contrib/junit.tpl" -o container-scan-report.xml'
