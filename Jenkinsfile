      ...
      stage("Trivy Scan") {
          steps {
              script {
                        ...
                        */
                        sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v /var/lib/jenkins/trivycache/:/root/.cache/ aquasec/trivy --exit-code 0 --no-progress tobiasparaiso/trivy:"$BUILD_ID"'
                    }
                }   
            }  
